...
class APKLeaks:
    def __init__(self, args):
        ...
        self.output = (
            tempfile.mkstemp(
                suffix=".%s" % ("json" if self.json else "txt"), prefix=self.prefix
            )[1]
        ...
        self.pattern = (
            os.path.join(str(Path(self.main_dir).parent), "config", "regexes.json")
            if args.pattern is None
            else args.pattern
        )
        self.jadx = (
            which("jadx")
            if which("jadx") is not None
            else os.path.join(
                str(Path(self.main_dir).parent),
                "jadx",
                "bin",
                "jadx%s" % (".bat" if os.name == "nt" else ""),
            ).replace("\\", "/")
        )
        self.out_json = {}
        self.scanned = False
        ...
        try:
            with closing(urlopen(exter)) as jadx:
                with ZipFile(io.BytesIO(jadx.read())) as zfile:
                    zfile.extractall(
                        os.path.join(str(Path(self.main_dir).parent), "jadx")
                    )
    ...
    def cleanup(self):
        ...
        if self.scanned:
            self.fileout.write(json.dumps(self.out_json, indent=4) if self.json else "")
            self.fileout.close()
            print(
                "%s\n** Results saved into '%s%s%s%s'%s."
                % (col.HEADER, col.ENDC, col.OKGREEN, self.output, col.HEADER, col.ENDC)
            )
            return
        ...
        # Retry delete because Windows may delay handle release
        for _ in range(5):
            try:
